...
const STORAGE_KEYS = {
  SEARCH: "foomatic_search",
  MANUFACTURER: "foomatic_manufacturer",
  DRIVER_TYPE: "foomatic_driver_type",
  MECHANISM_TYPE: "foomatic_mechanism_type",
  SUPPORT_LEVEL: "foomatic_support_level",
  COLOR_CAPABILITY: "foomatic_color_capability",
  PAGE: "foomatic_page",
  ITEMS_PER_PAGE: "foomatic_items_per_page",
} as const
    ...
    <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-6">
      <div className="flex items-center gap-2">
        ...
        <select
          ...
          className="px-3 py-1.5 rounded-md border border-border/50 bg-background text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-primary/50"
        >
...
export default function FoomaticPage() {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      const savedManufacturer = sessionStorage.getItem(STORAGE_KEYS.MANUFACTURER)
      const savedDriverType = sessionStorage.getItem(STORAGE_KEYS.DRIVER_TYPE)
      const savedMechanismType = sessionStorage.getItem(STORAGE_KEYS.MECHANISM_TYPE)
      const savedSupportLevel = sessionStorage.getItem(STORAGE_KEYS.SUPPORT_LEVEL)
      const savedColorCapability = sessionStorage.getItem(STORAGE_KEYS.COLOR_CAPABILITY)
      const savedPage = sessionStorage.getItem(STORAGE_KEYS.PAGE)
      const savedItemsPerPage = sessionStorage.getItem(STORAGE_KEYS.ITEMS_PER_PAGE)
  ...
  useEffect(() => {
    ...
    sessionStorage.setItem(STORAGE_KEYS.MANUFACTURER, selectedManufacturer)
    sessionStorage.setItem(STORAGE_KEYS.DRIVER_TYPE, selectedDriverType)
    sessionStorage.setItem(STORAGE_KEYS.MECHANISM_TYPE, selectedMechanismType)
    sessionStorage.setItem(STORAGE_KEYS.SUPPORT_LEVEL, selectedSupportLevel)
    sessionStorage.setItem(STORAGE_KEYS.COLOR_CAPABILITY, selectedColorCapability)
    sessionStorage.setItem(STORAGE_KEYS.PAGE, currentPage.toString())
    sessionStorage.setItem(STORAGE_KEYS.ITEMS_PER_PAGE, itemsPerPage.toString())
  }, [
    searchQuery,
    selectedManufacturer,
    selectedDriverType,
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="text-center py-20 mb-16 animate-fade-in">
          ...
          <h1 className="text-6xl lg:text-8xl font-black tracking-tighter mb-8 leading-tight">
            <span className="text-foreground">Foomatic</span>
            <br />
          ...
          <p className="text-xl text-muted-foreground max-w-4xl mx-auto leading-relaxed mb-12 font-light">
            Discover and explore printers from the comprehensive Foomatic database maintained by the
            OpenPrinting community. Find drivers, specifications, and compatibility information for
            seamless Linux printing.
          </p>
        ...
        {error ? (
          <div className="text-center py-24 animate-fade-in">
            ...
            <p className="text-muted-foreground text-lg max-w-md mx-auto leading-relaxed">{error}</p>
          </div>
        ) : loading ? (
          ...
          <div className="text-center py-24 animate-fade-in">
            ...
            <p className="text-muted-foreground text-lg max-w-md mx-auto leading-relaxed">
              Try adjusting your search terms or filter criteria to find the printer you&apos;re
              looking for.
            </p>
          </div>
        )}
