...
export default function PrinterCard({ printer }: PrinterCardProps) {
  ...
  return (
    <Link href={`/foomatic/printer/${printerId}`} className="h-full">
      <div className="group h-full flex flex-col rounded-xl border border-border bg-card transition-all duration-300 hover:border-border/80 hover:bg-accent/50 card-glow">
        <div className="p-5 pb-3">
          <div className="flex items-start gap-3">
            <div className="p-2 rounded-lg bg-muted border border-border group-hover:border-blue-400/30 group-hover:bg-blue-500/10 transition-colors">
              <PrinterIcon className="h-5 w-5 text-muted-foreground group-hover:text-blue-400 transition-colors" />
            </div>
            <div className="flex-1 min-w-0">
              <h3 className="text-base font-semibold text-foreground group-hover:text-blue-400 transition-colors duration-200 line-clamp-2 tracking-tight">
                {printer.model}
              </h3>
