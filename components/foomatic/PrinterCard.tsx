...
export default function PrinterCard({ printer }: PrinterCardProps) {
    ...
    switch (status.toLowerCase()) {
      case "perfect":
        return {
          ...
          className: "bg-green-500/20 text-green-700 dark:text-green-300 border-green-400/30",
        }
      case "partial":
      case "mostly":
        return {
          variant: "secondary" as const,
          className: "bg-yellow-500/20 text-yellow-700 dark:text-yellow-300 border-yellow-400/30",
        }
      case "unsupported":
        return {
          variant: "secondary" as const,
          className: "bg-red-500/20 text-red-700 dark:text-red-300 border-red-400/30",
        }
      default:
        return {
          variant: "secondary" as const,
          className: "bg-gray-500/20 text-gray-600 dark:text-gray-300 border-gray-400/30",
        }
    }
  }
  ...
  return (
    <Link href={`/foomatic/printer/${printerId}`} className="h-full">
      <Card className="h-full flex flex-col bg-gradient-card border border-border/50 rounded-2xl shadow-card backdrop-blur-sm group transition-all duration-300 hover:border-primary/50 hover:shadow-elegant">
