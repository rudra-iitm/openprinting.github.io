...
async function splitPrintersData() {
	try {
		...
		for (let i = 0; i < data.printers.length; i += batchSize) {
			...
			if (processed % 500 === 0) {
				console.log(
					`Processed ${processed}/${data.printers.length} printers...`,
				);
			}
		}

		console.log(
			`Successfully split ${data.printers.length} printers into individual files`,
		);
		...
		const savings = (((originalSize - mapSize) / originalSize) * 100).toFixed(
			1,
		);

		console.log("Size comparison:");
		console.log(
			`  Original printers.json: ${(originalSize / 1024 / 1024).toFixed(2)} MB`,
		);
		console.log(
			`  Lightweight printersMap.json: ${(mapSize / 1024).toFixed(2)} KB`,
		);
